...
export const filterOrder = async (
  ...
  if (responseData && responseData.Orders) {
    ...
    } else if (type === "Awaiting Processing") {
      newData = responseData.Orders.filter(
        (item) => item.status === "Awaiting Processing"
      );
      dispatch({ type: "fetchOrderAndChangeState", payload: newData });
      setDropdown(!dropdown);
    } else if (type === "Order Declined") {
      newData = responseData.Orders.filter(
        (item) => item.status === "Order Declined"
      );
      dispatch({ type: "fetchOrderAndChangeState", payload: newData });
      setDropdown(!dropdown);
    } else if (type === "Order Confirmed") {
      newData = responseData.Orders.filter((item) => item.status === "Order Confirmed");
      dispatch({ type: "fetchOrderAndChangeState", payload: newData });
      setDropdown(!dropdown);
    } else if (type === "Organ Dispatched") {
      newData = responseData.Orders.filter(
        (item) => item.status === "Organ Dispatched"
      );
      dispatch({ type: "fetchOrderAndChangeState", payload: newData });
      setDropdown(!dropdown);
    } else if (type === "Organ Delivered") {
      newData = responseData.Orders.filter(
        (item) => item.status === "Organ Delivered"
      );
      dispatch({ type: "fetchOrderAndChangeState", payload: newData });
      setDropdown(!dropdown);
    }
